...
//! #[tokio::main]
//! async fn main() {
//!     // Set panic hook so we can see the panic even if terminal was being
//!     // used in raw mode.
...
//!         // Drop the screen handle. IMPORTANT. If we don't drop, it will
//!         // block things like the screen renderer.
//!         drop(screen);
//!
//!         // Check if a key was pressed. We'll wait until an event happens. If
//!         // a key was pressed or an error happened, we exit. The only event
//!         // that makes this `if` fail is a resize event, so, we have to
